...
function form_id_to_user($user)
{
    ...
    if($user){
        foreach ($user as $v) {
            ...
            $str = $str.'<tr><td ><input type="checkbox" name="ids" class="check" value="'.$data['id'].'" /></td><td>'.$data['id'].'</td><td>'.$data['username'].'</td><td>'.$data['nickname'].'</td><td>'.$data['mobile'].'</a></td><td>'.get_user_status($data['status']).'</td></tr>';
        }
    }
    return $str;
}
